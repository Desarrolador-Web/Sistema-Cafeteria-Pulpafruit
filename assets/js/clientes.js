class Clientes {
    ...
    mostrarIniciosSesion() {
      axios
        .get(
          "http://localhost/Sistema-Cafeteria-Pulpafruit/controllers/clientesController.php?option=iniciosSesion"
        )
        .then((response) => {
          if (response.data.tipo === "success") {
            const datos = response.data.data;
            const contenidoTabla = datos
              .map(
                (fila) => `
                          <tr>
              ...
              .join("");
            document.getElementById("tablaIniciosSesionBody").innerHTML =
              contenidoTabla;
            new bootstrap.Modal(
              document.getElementById("modalIniciosSesion")
            ).show();
          } else {
    ...
    mostrarUsuariosRegistrados() {
      axios
        .get(
          "http://localhost/Sistema-Cafeteria-Pulpafruit/controllers/clientesController.php?option=usuariosRegistrados"
        )
        .then((response) => {
          if (response.data.tipo === "success") {
            const contenidoTabla = response.data.data
              .map(
                (usuario) => `
                          <tr>
            ...
            document.getElementById("tablaUsuariosBody").innerHTML =
              contenidoTabla;
            new bootstrap.Modal(
              document.getElementById("modalUsuariosRegistrados")
            ).show();
          } else {
            alert("Error al cargar los datos: " + response.data.mensaje);
          }
    ...
    mostrarProductosAgotados() {
      axios
        .get(
          "http://localhost/Sistema-Cafeteria-Pulpafruit/controllers/clientesController.php?option=productosAgotados"
        )
        .then((response) => {
          if (response.data.tipo === "success") {
            const contenidoTabla = response.data.data
              ...
              .join("");
            document.getElementById("tablaProductosAgotadosBody").innerHTML =
              contenidoTabla;
            new bootstrap.Modal(
              document.getElementById("modalProductosAgotados")
            ).show();
          } else {
    ...
    mostrarNivelarInventario() {
      axios
        .get(
          "http://localhost/Sistema-Cafeteria-Pulpafruit/controllers/clientesController.php?option=nivelarInventario"
        )
        .then((response) => {
          if (response.data.tipo === "success") {
            const datos = response.data.data;
            ...
            document.getElementById("tablaNivelarInventarioBody").innerHTML =
              contenidoTabla;
    ...
    moverConEnter(event) {
      if (event.key === "Enter") {
        ...
        let siguienteCelda =
          celdaActual.parentElement.cells[celdaActual.cellIndex + 1];
    ...
    enviarEdicion(
      ...
      axios
        .post(
          "http://localhost/Sistema-Cafeteria-Pulpafruit/controllers/clientesController.php?option=actualizarProducto",
          formData
        )
        .then((response) => {
          if (response.data.tipo === "success") {
    ...
    mostrarDescargarInforme() {
        axios
          .get(
            "http://localhost/Sistema-Cafeteria-Pulpafruit/controllers/clientesController.php?option=descargarInforme"
          )
          .then((response) => {
            if (response.data.tipo === "success") {
              const contenidoTabla = response.data.data
              ...
              // Insertar el contenido en el tbody
              document.getElementById("tablaInformeBody").innerHTML = contenidoTabla;
      
              // **Destruir** cualquier instancia previa de DataTables si existe
              if ($.fn.DataTable.isDataTable("#tablaInforme")) {
                $("#tablaInforme").DataTable().destroy();
              }
      
              // **Reinicializar** DataTables
              $("#tablaInforme").DataTable({
                paging: true, // Activa paginación
                searching: true, // Activa búsqueda
                ordering: true, // Permite ordenar columnas
                responsive: true, // Diseño adaptable
                language: {
                  url: "//cdn.datatables.net/plug-ins/1.10.25/i18n/Spanish.json",
                },
                dom: "Bfrtip", // Ubicación de botones, filtro, tabla, info y paginación
                buttons: [
                  // Si quieres, también puedes agregar exportaciones
                  {
                    extend: "pdfHtml5",
  ...
  document.getElementById("descargarPDF").addEventListener("click", function () {
    window.open(
      "http://localhost/Sistema-Cafeteria-Pulpafruit/controllers/clientesController.php?option=generarInformePDF",
      "_blank"
    );
  });
  ...
  document.addEventListener("DOMContentLoaded", function () {
    document
      .getElementById("metric-inicios-sesion")
      .addEventListener("click", () => cliente.mostrarIniciosSesion());

    document
      .getElementById("metric-usuarios-registrados")
      .addEventListener("click", () => cliente.mostrarUsuariosRegistrados());
    document
      .getElementById("metric-productos-agotados")
      .addEventListener("click", () => cliente.mostrarProductosAgotados());
    document
      .getElementById("metric-nivelar-inventario")
      .addEventListener("click", () => cliente.mostrarNivelarInventario());
    document
      .getElementById("metric-descargar-informe")
      .addEventListener("click", () => cliente.mostrarDescargarInforme());
  });
